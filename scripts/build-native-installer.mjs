...
async function innoSetupCompiler() {
  const candidates = [
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Try the next known per-machine compiler location.
    }
  }
  throw new Error("Inno Setup 6.3+ is required to build the graphical Windows installer. Set INNO_SETUP_COMPILER to ISCC.exe.");
}
...
if (target.startsWith("win32-")) {
  if (process.platform !== "win32") throw new Error("Windows graphical installers must be compiled on Windows.");
  const runtimeDirectory = path.join(releaseDirectory, "staging", target, "runtime");
  const runtimeMetadata = JSON.parse(await readFile(path.join(runtimeDirectory, "bundle.json"), "utf8"));
  if (runtimeMetadata.version !== version || runtimeMetadata.target !== target || runtimeMetadata.product !== "socium") {
    throw new Error("Staged runtime does not match this Windows installer build.");
  }
  const compiler = await innoSetupCompiler();
  await run(compiler, [
    "/Qp",
    `/DMyAppVersion=${version}`,
    `/DMyTarget=${target}`,
    `/DRuntimeDir=${runtimeDirectory}`,
    `/DOutputDir=${releaseDirectory}`,
    `/DOutputBaseFilename=${path.parse(installerName).name}`,
    `/DIconFile=${path.join(projectRoot, "src", "app", "favicon.ico")}`,
    `/DLicenseFile=${path.join(projectRoot, "LICENSE")}`,
    `/DManifestUrl=${manifestUrl}`,
    path.join(projectRoot, "packaging", "windows", "SociumSetup.iss"),
  ]);
  ...
  const binary = path.join(projectRoot, "native", "installer", "target", "release", "socium-native-installer");
  await access(binary);
