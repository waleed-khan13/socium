...
def test_draft_version_approval_and_single_publish(client, monkeypatch) -> None:
    ...
    async def fake_generate(*_args, **_kwargs):
        return GeneratedContent(
            title="Local launch",
            body="A factual post generated for review. Review the release notes.",
            hashtags=["#local", "#Socium"],
            call_to_action="Review the release notes.",
            image_prompt="A dark editorial product workspace with amber lighting",
            image_negative_prompt="watermark, distorted text",
            image_alt_text="Dark product workspace prepared for a release review",
    ...
    assert post["callToAction"] == "Review the release notes."
    assert post["imagePrompt"].startswith("A dark editorial")
    assert post["imageNegativePrompt"] == "watermark, distorted text"
    assert post["imageAltText"].startswith("Dark product workspace")
    assert post["brandProfileVersion"] == generated.json()["state"]["workspace"]["profileVersion"]
    ...
    edited = client.patch(
        ...
        json={
            ...
            "imagePrompt": "A revised campaign visual",
            "imageNegativePrompt": "blur",
            "imageAltText": "Revised campaign visual for the launch",
        },
    )
    assert edited.status_code == 200
    ...
    assert revised["callToAction"] == "Read the guide."
    assert revised["imagePrompt"] == "A revised campaign visual"
...
def test_local_media_library_uploads_deduplicates_transforms_and_deletes(client) -> None:
    ...
    generated_duplicate = create_generated_media_asset(
        source_bytes,
        prompt="A green campaign card",
        negative_prompt="watermark",
        alt_text="Generated green campaign card",
        provider_kind="openai-images",
        model="test-image-model",
        parameters={"preset": "landscape"},
    )
    assert generated_duplicate["deduplicated"] is True
    assert generated_duplicate["asset"]["id"] == asset["id"]
    assert generated_duplicate["asset"]["altText"] == "Generated green campaign card"
...
def test_media_generation_runs_through_durable_worker(client, monkeypatch) -> None:
    ...
    queued = client.post(
        ...
        json={
            "prompt": "A durable queued campaign visual",
            "altText": "A cyan campaign visual generated by the durable worker",
            "preset": "landscape",
        },
    )
    ...
    generated_asset = next(item for item in assets if item["id"] == completed["resultRef"])
    assert generated_asset["altText"] == "A cyan campaign visual generated by the durable worker"
