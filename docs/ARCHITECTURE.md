...
Each `posts` row stores that complete content kit plus the exact confirmed brand-profile revision used to create it. Editing any part increments the post revision and invalidates approval. The dashboard exposes the kit for review and can prefill Media Studio; when the operator generates the visual, planned alt text follows the request into the resulting local asset. Referenced logo and image bytes remain local unless a separate operator-approved media workflow explicitly uses them.

`approval_actions` is the durable boundary for Telegram and Slack decisions. A row binds an opaque one-time ID to the post, exact revision, transport, 72-hour expiry, remote message reference, selected action, and terminal state. Approve, Skip, Edit, and Regenerate all recheck that row and the current post under `BEGIN IMMEDIATE`; content changes supersede remaining actions for the old revision. Regenerate performs the provider call outside the transaction and commits only if the claimed action and post revision are still current. A remote Edit writes a durable local handoff that the dashboard acknowledges after opening the exact revision. Startup marks interrupted delivery/regeneration records failed so they can be resent deliberately rather than replayed silently.
...
- Slack account health checks call `auth.test` for the bot token and `apps.connections.open` for the app token. While a Slack approval is pending, at most one verified account runs a supervised Socket Mode connection; with no pending action there is no socket worker. Every envelope is acknowledged before a configured-channel, revision-bound decision is applied.
