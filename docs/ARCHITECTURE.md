...
| Image AI | Capability of the verified primary AI connection | One setup and credential drives copy plus images; no second provider configuration surface. |
| Desktop integration | Small Rust helper on Windows | Native tray, folder picker, `.lnk` shortcuts, and login startup without PowerShell at runtime. |
...
- Persist schedules before acknowledging them. If the computer is off or scheduling is paused, mark overdue publication as missed and require an explicit Run now, Reschedule, or Skip decision after restart.
...
Generation always receives the basic business name and description for compatibility with existing installations. Expanded brand fields are added only when the stored profile is both complete and explicitly confirmed. The prompt names the confirmed revision and treats restricted claims as prohibitions. The provider must return a structured content kit: title, channel-ready body, hashtags, call to action, image prompt, visual exclusions, planned alt text, and rationale. FastAPI parses that untrusted response, normalizes and deduplicates hashtags, applies per-channel body and hashtag limits, ensures the CTA appears once in published copy, fills safe visual fallbacks, and rejects output containing an explicitly restricted claim.

Each `posts` row stores that complete content kit, its generated `media_asset_id`, and the exact confirmed brand-profile revision used to create it. A post is not sent for approval until both structured copy and a verified local image asset exist. Editing or regenerating either part increments the post revision and invalidates approval. Referenced logo and image bytes remain local unless a connector upload is explicitly required for the approved destination.

`approval_actions` is the durable boundary for Telegram and Slack decisions. A row binds an opaque one-time ID to the post, exact revision, transport, 72-hour expiry, remote message reference, selected action, and terminal state. Approve, Skip, Edit, Regenerate post, and Regenerate image all recheck that row and the current post under `BEGIN IMMEDIATE`; content changes supersede remaining actions for the old revision. Post regeneration preserves the existing media asset, while image regeneration preserves the existing copy. Provider work happens outside the transaction and commits only if the claimed action and post revision are still current. A remote Edit writes a durable local handoff that the dashboard acknowledges after opening the exact revision. Startup marks interrupted delivery/regeneration records failed so they can be resent deliberately rather than replayed silently.

First-run onboarding is an orchestration layer over these existing boundaries rather than a second settings system. Its order is storage, Business Knowledge, then AI: storage must exist before durable knowledge can be written, while website analysis can prepare an editable evidence-based profile without an AI connection. `app_metadata` stores the current wizard step, storage snapshot, dismissal, and completion timestamps. Storage confirmation is tied to the active data/model paths; provider verification is tied to the saved kind, endpoint, model, and settings timestamp; knowledge readiness uses the confirmed profile revision. The completion transition rechecks all three inside a write transaction. Dismissal hides the modal but preserves its step, and starting again clears only the dismissal marker.

Saving the reviewed business profile promotes its non-empty values into confirmed `knowledge_items`. The public state exposes deterministic content defaults derived only from that confirmed context. Content generation resolves any omitted topic, tone, or objective from the same context at the backend boundary, so automations and UI clients cannot drift into separate prompt defaults.

The local scheduler is a lightweight event-and-deadline supervisor. It waits indefinitely when no deadline exists, wakes when durable work changes, and starts at most one worker for one leased SQLite job. The worker has a bounded lifetime and exits after writing a terminal or retry state. Lease tokens and expiries prevent stale owners from completing recovered work; safe preflight and read-only failures use bounded exponential backoff, while three repeated supervisor crashes stop execution in `Needs attention` until the operator resumes it. On startup and resume, overdue publication becomes `missed` and cannot run until the operator selects Run now, Reschedule, or Skip for the still-approved exact revision. Once a Telegram, WordPress, Facebook Page, Instagram, LinkedIn Member, or LinkedIn Company Page delivery has been reserved and attempted, an uncertain response becomes a failed review item instead of an automatic retry; the operator must explicitly review it before another attempt.

`automation_rules` stores recurring intent separately from generated content: destination, topic, tone, objective, IANA timezone, exact weekdays, local publish time, generation lead time, approval transports, enabled state, and the next generation/publication deadlines. Only one idempotent `automation.generate:{rule}:{occurrence}` job exists for an occurrence. It creates a new post revision, sends optional Telegram/Slack approval actions, advances the rule to its next occurrence, and then returns the worker to idle. An approved automation post receives a revision-bound `post.publish` job for its intended target time; a late approval uses the current time. Pausing, editing, duplicating, or deleting a rule cancels only future generation jobs. Posts retain their history, and deletion nulls the optional rule reference instead of cascading into content. A generation left due while the computer was off runs after restart but still stops at human approval, so catch-up cannot silently publish.

The Content Studio owns creation and embeds an asset library for storage and reuse. The asset library stores metadata in `media_assets` and bytes under the configurable data directory's `media/` folder; it is not an independent generation workflow. Uploads and provider-generated bytes are bounded before decoding, verified from content rather than declared MIME claims, restricted to JPEG/PNG/WebP, protected by pixel and dimension limits, named with generated UUIDs, and deduplicated by SHA-256. Previews and preset transforms are newly encoded WebP files. Content endpoints resolve only database-owned storage names inside the media directory and send `nosniff`; the list API never exposes filesystem paths. Metadata may contain an operator-supplied public HTTPS source, but Socium never treats a loopback content URL as remotely publishable.

Image generation derives its runtime from the single verified `provider_settings` row. Provider capability mapping selects the supported image endpoint and model without exposing a second credential form. Gemini uses native `generateContent` with image response modality; OpenAI uses its Images API. Legacy `image_provider_settings` data remains readable during migration but is not exposed as the active runtime. Providers without a same-connection image capability fail closed before an automated approval package is created.

Manual Media Studio requests still use the SQLite-backed `media.generate` worker, progress reporting, cancellation, retry, and verified ingestion. Automation instead calls the same verified adapter inside the bounded `automation.generate` job and persists the resulting asset with the post before approvals are issued. `media_generations` records successful outputs, while `media_assets` carries non-secret prompt, model, parameters, checksum, and local storage provenance.
...
The growth schema keeps that `leads` row as a compatibility projection while `companies` and `contacts` separate organization facts from recipient and compliance data. Migration `20260908_0024` backfills both references without replacing a lead ID, and every later import maintains the normalized rows in the same transaction. Campaigns store members and ordered templates separately. Activation creates a first `outreach_drafts` revision with no delivery side effect; exporting an approved step queues `campaign.prepare` for the next wait boundary. That job creates another review-required draft. A Gmail message from the recipient stops the sequence only when its timestamp is newer than campaign activation. Consent, suppression, and retention changes re-run the deterministic gate and stop matching members. Pausing a campaign cancels its queued preparation jobs; resuming requeues the same idempotency key rather than duplicating a step.
...
- Telegram uses `getUpdates` long polling only while an unexpired Telegram approval is awaiting a decision; with no pending action it sleeps without a timer. A public webhook is neither requested nor required.
- Slack, LinkedIn Member, and Gmail use a small Cloudflare Worker OAuth broker registered once by the Socium maintainer. The local app creates state plus S256 PKCE, the broker exchanges provider authorization, and a five-minute encrypted one-time handoff returns credentials directly to the loopback API. Slack interactions are signature-verified by the broker and leased to the local app through a hashed relay credential only while an approval is pending. Gmail refresh credentials remain in the local encrypted vault; the broker only performs a stateless refresh exchange. Tokens are never returned to the dashboard browser.
...
- LinkedIn Member publishing uses the maintainer-registered Socium app and explicit 3-legged OAuth consent with `openid`, `profile`, and `w_member_social`. The one-click flow retrieves `/v2/userinfo`, encrypts the token locally, and publishes only frozen approved text revisions through `/rest/posts`. Browser cookies, passwords, page scraping, and simulated engagement remain outside the connector boundary.
- Gmail uses the official Threads and Messages APIs with `gmail.readonly` and `gmail.send`. A bounded manual sync stores normalized threads in SQLite. AI generation is a recoverable background proposal; edits create a new revision, approval binds to that exact revision, and delivery is duplicate-safe and fails closed when Gmail does not confirm the message ID.
...
The primary stable installation uses the platform-native `.exe`, `.dmg`, or `.AppImage` from GitHub Releases. Each installer embeds the checksummed runtime, rejects unsafe archive paths, validates bundle metadata, and atomically records the active immutable runtime. The npm CLI remains an alternate installation and update path. FastAPI is shipped as a native executable with migrations embedded; Next.js is shipped as a standalone server with flattened production dependencies. Node.js, Python, Rust, uv, pnpm, Docker, and a source checkout are therefore not prerequisites for native-installer users.

Mutable state is never placed inside the versioned runtime. SQLite, `master.key`, media, exports, logs, checksum-backed backups, and the installation record live outside the active version under the native application-data root documented in [INSTALLATION.md](INSTALLATION.md). A stable launcher reads that record and starts the controller from the active version; the controller launches the bundled Node/FastAPI runtime on loopback and exposes a random-token loopback control endpoint to the backend and native tray only. On Windows, a signed-release-ready Rust helper owns the tray, native folder dialog, shortcuts, and login-start entry; Socium does not launch PowerShell for these runtime operations. The helper reads the random control token from the local runtime-state file instead of exposing it in a process argument. `socium update` backs up data, verifies and stages the new runtime, runs its migrations behind a temporary health endpoint, and atomically retains one rollback target. Normal uninstall removes runtimes and native launchers while preserving data unless the operator explicitly adds `--purge-data`.
