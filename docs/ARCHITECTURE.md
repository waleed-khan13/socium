...
- Slack and LinkedIn Member use a small Cloudflare Worker OAuth broker registered once by the Socium maintainer. The local app creates state plus S256 PKCE, the broker exchanges provider authorization, and a five-minute encrypted one-time handoff returns credentials directly to the loopback API. Slack interactions are signature-verified by the broker and leased to the local app through a hashed relay credential only while an approval is pending. Tokens are never returned to the dashboard browser.
