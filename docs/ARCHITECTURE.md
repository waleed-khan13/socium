...
First-run onboarding is an orchestration layer over these existing boundaries rather than a second settings system. `app_metadata` stores the current wizard step, storage snapshot, dismissal, and completion timestamps. Storage confirmation is tied to the active data/model paths; provider verification is tied to the saved kind, endpoint, model, and settings timestamp; brand readiness uses the confirmed profile revision. The completion transition rechecks all three inside a write transaction. Dismissal hides the modal but preserves its step, and starting again clears only the dismissal marker.
