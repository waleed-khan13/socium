...
| Gmail | Socium **Connect Gmail** button | Sign in and select **Allow** once. Socium requests read-only inbox access plus send access, stores the credentials in the encrypted local vault, and sends only an exact reply revision you approve. End users do not create or paste Google tokens. |
