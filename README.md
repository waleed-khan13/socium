...
2. Build **Business Knowledge** from a website or the editable profile, then select **Save & confirm profile**. Confirmation creates source-aware Knowledge facts and an auditable revision before any AI is required.
3. Choose **Local AI - Recommended** or **Cloud API**. Local setup inspects this computer, recommends a model, and downloads it with 1% progress when Ollama is running. Cloud setup needs only the selected provider's API key.
...
The **Content Studio** is the single creation surface. Confirmed Business Knowledge pre-fills its topic, tone, and objective; one generation request creates the post, hashtags, call to action, image direction, alt text, and image. Its embedded asset library accepts decoded JPEG, PNG, and WebP files up to 10 MB and 40 megapixels, and automatically shows generated assets. It uses random storage names, retains originals locally, creates bounded WebP previews, deduplicates identical bytes, and can create social-size variants. The asset library stores and reuses media; it is not a second content generator.
